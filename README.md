...
├── Solution.lean       ← the actual proof (754 lines, all lemmas inline)
├── config.json         ← comparator configuration
├── lakefile.toml       ← pinned Mathlib commit
...
- `lean4export` (kernel export tool, version-matched to Lean 4.24.0)
...
and `Solution.lean` compiles zero-`sorry` against the pinned Mathlib
commit in my own development environment.

If anyone with comparator already set up wants to run this and report
the result, I'd appreciate it. The intent is to provide a verifiable
artifact, not to ask anyone to do my work for me — happy to set up
WSL2 or a Linux VM and run it myself given a small amount of time;
this just lets community members with the tooling already in place
verify it faster than I can.
